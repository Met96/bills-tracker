    ...
    <div class="max-w-6xl mx-auto">
      ...
      <div v-if="!parsedBill" class="bg-white rounded-lg shadow p-8 mb-8">
        ...
        >
          ...
          <label class="px-6 py-2 bg-blue-800 text-white rounded-lg hover:bg-blue-900 transition cursor-pointer inline-block">
            {{ $t('upload.selectFile') }}
            <input
              type="file"
      ...
      <div v-if="parsedBill && !confirming" class="bg-white rounded-lg shadow p-8">
        ...
        <form class="space-y-6" @submit.prevent="confirmBill">
          ...
          <div class="flex gap-4 pt-6">
            <button
              type="button"
              class="flex-1 px-6 py-3 bg-gray-200 text-gray-900 rounded-lg hover:bg-gray-300 cursor-pointer transition font-medium"
              @click="resetForm"
            >
            ...
            <button
              type="submit"
              :disabled="confirming"
              class="flex-1 px-6 py-3 bg-green-600 text-white rounded-lg hover:bg-green-700 transition font-medium cursor-pointer disabled:opacity-50 disabled:cursor-not-allowed"
            >
      ...
      <div v-if="successMessage" class="bg-green-50 border border-green-200 rounded-lg p-8 text-center">
        ...
        <div class="flex gap-4 justify-center">
          <NuxtLink
            to="/"
            class="px-6 py-2 bg-green-600 text-white rounded-lg hover:bg-green-700 transition font-medium cursor-pointer"
          >
            {{ $t('upload.backToDashboard') }}
          </NuxtLink>
          <button
            class="px-6 py-2 bg-white border border-green-600 text-green-600 rounded-lg hover:bg-green-50 transition font-medium cursor-pointer"
            @click="resetForm"
          >
